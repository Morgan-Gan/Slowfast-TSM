...
def _get_model_analysis_input(cfg, use_train_input):
    ...
    model_inputs = pack_pathway_output(cfg, input_tensors)                                 # input_tensors:[3,8,224,224]
...
def get_model_stats(model, cfg, mode, use_train_input):
    ...
    inputs_list = inputs[0][0]
    inputs_list = inputs_list.squeeze(0).permute(1,0,2,3)
    # inputs = (inputs_list, inputs[1])
...
def launch_job(cfg, init_method, func, daemon=False):
    ...
    """
    ## multiprocessing 多进程包，Spawning子线程仅依赖于spawn启动方法，创建实例进程并调用join来等待他们完成
    if cfg.NUM_GPUS > 1:
        torch.multiprocessing.spawn(
            mpu.run,                                                       # fn函数称为派生进程入口点,fn(i, *args)i是进程索引，args是传递的参数元祖
            nprocs=cfg.NUM_GPUS,                                           # 派生进程数
            args=(                                                         # 传递给fn参数
                cfg.NUM_GPUS,
                func,
                init_method,
            ...
            daemon=daemon,                                                  # 派生进程守护进程标志，如果设置为true，将创建守护进程
        )
    else:
        func(cfg=cfg)
